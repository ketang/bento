...
`gh issue close`. They catch "built but not wired," "shipped but
undocumented," and "plan file rotting in the live directory" — failure modes
that landing proof alone does not catch.
...
1. **Wiring proof.** For each new capability the issue added (worker, provider
   ...
   so explicitly in the closing comment (`gh issue close <id> -c "... wiring
   tracked in #<follow-up-number>"`).
...
Skipping a check requires saying why in the closing comment (e.g. "no plan
file existed," "change is internal-only, no docs to sync").
