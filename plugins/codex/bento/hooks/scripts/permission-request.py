#!/usr/bin/env python3
"""Codex PermissionRequest/Bash hook: auto-allow Bash invocations of Python
scripts shipped by this plugin, so users are not re-prompted for the plugin's
own bundled helpers after the plugin-install trust check.

Usage (as wired from the Codex peer hooks.json):
    permission-request.py <plugin-name> <plugin-root>
...
Decision logic:
  ...
  * The resolved script must be a regular .py file under either the plugin
    root or the plugin's own source repo (detected by an ancestor with a
    matching plugins/codex/<name>/.codex-plugin/plugin.json).

On allow, prints the PermissionRequest decision JSON on stdout.
...
def _find_source_repo_root(script_path: Path, plugin_name: str) -> Path | None:
    ...
    for _ in range(SOURCE_REPO_WALK_DEPTH):
        plugin_meta = (
            parent / "plugins" / "codex" / plugin_name / ".codex-plugin" / "plugin.json"
        )
        if plugin_meta.is_file():
            try:
...
def main(argv=None, stdin=None, stdout=None, stderr=None) -> int:
    ...
    if len(argv) != 3:
        print(
            f"permission-request: expected 2 args (plugin-name, plugin-root); got {len(argv) - 1}",
            file=stderr,
        )
        return 0
    ...
    try:
        ...
        print(f"permission-request: invalid hook input JSON: {exc}", file=stderr)
        return 0

    command = (payload.get("tool_input") or {}).get("command") or ""
    decision, reason = decide(command, plugin_name, plugin_root)

    print(f"permission-request[{plugin_name}]: {reason}", file=stderr)
