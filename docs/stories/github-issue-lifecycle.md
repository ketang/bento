...
- Issue closure happens only after verified landing: the merge SHA must be an ancestor of the integration branch, and that branch must be confirmed pushed to the remote. Both checks apply even when the closing agent just ran land-work itself.
- The closing comment names the merge SHA and the branch it landed on.
