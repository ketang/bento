...
## Story
An agent is deep into a multi-step implementation task when the context window approaches compaction limits. The user invokes `/handoff`. The skill checks all three preconditions — the current directory is inside a git repo, HEAD resolves to a named branch, and no active expedition is detected in this worktree — then gathers: the current branch name, a summary of in-flight work, the next concrete action, any open questions, decisions made, files changed, and a "do not redo" list of already-completed steps. It writes this into a markdown file under `/tmp/` whose name derives from the `--slug` contract, then echoes the full contents back to the chat. The user copies the file path or contents into a new session prompt, and the next agent resumes with full context.

## Expected Behavior
- The skill requires the working directory to be inside a git repo on a named branch.
- It writes a markdown file to `/tmp/` under the headings the resolved template provides.
...
- The SKILL.md bundles a default template of seven labeled slots, and states the on-disk template is editable: a repo- or home-scope override "may add, remove, rename, or reorder headings".
- The SKILL.md states the skill "operates only when all three preconditions hold": inside a git repository, HEAD on a named branch, and no active expedition detected.
