...
5. When a repo-local `audit` skill was generated, commit it and record its
...
```
`.claude/skills/audit/` is generated by `bento:generate-audit`; regenerate when
the repo's stack, build/test commands, or risk surfaces change materially.
```
